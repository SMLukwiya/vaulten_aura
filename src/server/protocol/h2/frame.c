 ...
 *
 */
uint8_t *aura_encode_frame_header(uint8_t *dest, size_t frame_len, uint8_t type, uint8_t flags, uint32_t stream_id) {
...
uint8_t *aura_encode_control_frame(struct aura_sliding_buf *buf, uint8_t type, uint8_t flags, uint32_t stream_id,
    ...
    if (!aura_sliding_buffer_ensure_capacity(buf, frame_len)) {
        return NULL;
    }
...
int aura_produce_header_frame(struct aura_sliding_buf *buf, uint32_t stream_id, uint8_t type,
    ...
    dest = aura_sliding_buffer_write_pointer(buf);
    ...
    if (payload && payload_len > 0) {
        if (aura_sliding_buffer_append(buf, payload, payload_len) < 0) {
            return -1;
        }
    }
...
int aura_encode_data_frame(struct aura_sliding_buf *buf, uint32_t stream_id, uint8_t flags,
    ...
    if (!aura_sliding_buffer_ensure_capacity(buf, payload_len + A_H2_FRAME_HEADER_SIZE))
        return -1;

    dest = aura_sliding_buffer_write_pointer(buf);
    dest = aura_encode_frame_header(dest, payload_len, A_H2_FRAME_TYPE_DATA, flags, stream_id);
    aura_sliding_buffer_commit_write(buf, A_H2_FRAME_HEADER_SIZE);
    if (payload && payload_len > 0) {
        if (aura_sliding_buffer_append(buf, payload, payload_len) < 0)
            return -1;
