...
const char fn_start_msg[] = "\x1B[1;32mFunction started successfully\x1B[0m";
const char fn_start_error[] = "\x1B[1;31mFunction start failed\x1B[0m";
const char fn_start_not_exist[] = "\x1B[1;31mFunction start failed. Provided function does not exist!\x1B[0m";
    ...
    while (true) {
        ...
        if (evt->msg_len > 0) {
            ...
            case A_FN_ERROR_GENERIC:
                app_info(false, 0, fn_start_error);
                break;

            case A_FN_ERROR_NOT_EXIST:
                app_info(false, 0, fn_start_not_exist);
                break;

            case A_FN_ERROR_NONE:
                app_info(false, 0, "%s", fn_start_msg);
                break;
