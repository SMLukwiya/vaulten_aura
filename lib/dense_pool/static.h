...
#define A_DEFINE_DENSE_POOL(name, size, struct_type)                                                                       \
    ...
    static inline void aura_##name##_dense_pool_init(struct aura_##name##_dense_pool *pool) {                              \
        ...
        for (int i = 0; i < size; ++i)                                                                                     \
            ...
            pool->free_stack[i] = size - 1 - i;                                                                            \
                                                                                                                           ...
                                                                                                                           \
    /* Initialize dense pool with caller function */                                                                       \
    static inline void aura_##name##_dense_pool_init2(struct aura_##name##_dense_pool *pool, int (*fn)(struct_type *)) {   \
        memset(pool, 0, sizeof(*pool));                                                                                    \
                                                                                                                           \
        pool->free_top = (size);                                                                                           \
        for (int i = 0; i < size; ++i) {                                                                                   \
    ...
    aura_##name##_dense_pool_release(struct aura_##name##_dense_pool *pool, uint32_t idx) {                                \
        aura_dense_pool_core_release(pool->free_stack, &pool->free_top, idx);                                              \
                                                                                                                           ...
                                                                                                                           \
    /**                                                                                                                    \
     * Returns a pointer to the underlying pool entries array                                                              \
     */                                                                                                                    \
    static inline struct_type *aura_##name##_dense_pool_get_entries(struct aura_##name##_dense_pool *pool) {               \
        return pool->slots;                                                                                                \
                                                                                                                           ...
                                                                                                                           \
    /**                                                                                                                    \
     * Run the provided function for each item in the pool.                                                                \
     * Returns 0 on success or breaks out of the loop with negative                                                        \
     * error code.                                                                                                         \
     */                                                                                                                    \
    static inline int aura_##name##_dense_pool_for_each(struct aura_##name##_dense_pool *pool, int (*fn)(struct_type *)) { \
        int rv = 0;                                                                                                        \
        for (int i = 0; i < size; ++i) {                                                                                   \
            rv = fn(&pool->slots[i]);                                                                                      \
            if (rv < 0)                                                                                                    \
