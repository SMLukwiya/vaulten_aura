...
static struct aura_db_brokered_request *a_db_brokered_req_create(struct aura_mem_ctx *mc, ns_t ns,
...
static inline void a_db_brokered_req_destroy(struct aura_db_brokered_request *req) {
    if (req)
        aura_free(req);
}
...
int aura_db_brokered_fetch(struct aura_mem_ctx *mc, ns_t ns, schema_id_t schema_id,
    ...
    aura_db_request_dump(request);
    a_init_msg_hdr(msg_hdr, len, A_MSG_CMD_EXECUTE, A_CMD_DB_FETCH_REQUEST);
    rv = aura_msg_send(dmn_fd, &msg_hdr, (void *)request, len, -1);
    a_db_brokered_req_destroy(request);
    if (rv < 0)
        return -1;

    rv = aura_recv_resp(out_data, dmn_fd, mc);
    if (rv < 0)
        return -1;
...
void aura_db_request_dump(struct aura_db_brokered_request *req) {
    ...
    app_debug(true, 0, "    data: %s", req->data.base);
}